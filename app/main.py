...
def ready(store: ConversationStore = Depends(get_store)):
    ...
    # 1. Instance đang drain → không nhận traffic mới
    if lifecycle.shutting_down:
        return JSONResponse(
            status_code=503,
            content={
    ...
    # 2. Redis chết → app không thể phục vụ request bình thường
    if not store.ping():
        return JSONResponse(
            status_code=503,
            content={
...
def ask(
    ...
    """
        # 1. Rate limit phải check trước khi gọi LLM
    limiter.check(user_id)

    # 2. Kiểm tra ngân sách trước khi gọi LLM
    guard.check(user_id)

    # 3. Lấy lịch sử hội thoại hiện tại
